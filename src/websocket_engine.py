...
class ExchangeStream:
    ...
    def __init__(self, symbols: List[str], callback: Callable[[TickerData], Awaitable[None]]):
            ...
            async with session.ws_connect(url) as ws:
                async for msg in ws:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        # Normalizacija simbola: SOLUSDT -> SOL/USDT
                        raw_s = data['s'].upper()
                        ...
                        ticker = TickerData(
                            ...
                            timestamp=time.time() # Binance nema dobar timestamp u bookTickeru, koristimo local
                        )
                        # PUSH EVENT
...
class WebSocketEngine:
    def __init__(self, exchanges: List[str], coins: List[str], logger, strategy_callback):
        self.exchanges = exchanges
        self.coins = coins
        self.logger = logger
        self.strategy_callback = strategy_callback # Ovo je funkcija iz StrategyEngine
        self.running = False
        self.tasks = []

    async def _relay_ticker(self, ticker: TickerData):
        """Prima podatak od streama i odmah ga šalje u Strategiju."""
    ...
    async def start(self):
        ...
        streams = []
        if 'binance' in self.exchanges: streams.append(BinanceStream(self.coins, self._relay_ticker))
        if 'bybit' in self.exchanges: streams.append(BybitStream(self.coins, self._relay_ticker))
        if 'okx' in self.exchanges: streams.append(OkxStream(self.coins, self._relay_ticker))
            
        self.logger.info("⚡ WS Engine Starting...")
    ...
    async def _keep_alive(self, stream, session):
        while self.running:
            try:
                ...
                self.logger.error(f"Stream disconnected: {e}. Reconnecting...")
